...
export async function cancelOrderController(request, response) {
  try {
    ...
    if (order.order_status === "delivered" || order.order_status === "cancelled") {
      return response.status(400).json({
        message: `Cannot cancel order - order status is ${order.order_status}`,
        error: true,
        success: false,
      });
    }
    ...
    const updatedOrder = await OrderModel.findByIdAndUpdate(orderId, { order_status: "cancelled" }, { new: true });
